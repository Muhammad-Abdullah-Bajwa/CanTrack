...
class ProvinceAdapter (private val list: MutableList<CovidData>) :
    ...
    inner class ProvinceViewHolder(val binding: ProvinceItemLayoutBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProvinceViewHolder {
        return ProvinceViewHolder(ProvinceItemLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false))
    }

    override fun onBindViewHolder(holder: ProvinceViewHolder, position: Int) {
        holder.binding.provinceName.text = list[position].province
        holder.binding.provinceCases.text = NumberFormat.getInstance().format(list[position].cumulative_cases)
